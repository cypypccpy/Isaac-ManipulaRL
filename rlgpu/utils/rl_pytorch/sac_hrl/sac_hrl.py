...
from utils.rl_pytorch.sac_hrl import ReplayBeffer, MetaActorCritic
...
class SAC:
    ...
    def __init__(self,
                ...
                #  demonstration_buffer_len = 50000,
                 demonstration_buffer_len = 120000,
        ...
        self.meta_actor_critic = MetaActorCritic
        self.meta_actor_critic.to(self.device)
        ...
        if self.is_testing:
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                for _ in range(self.num_learning_epochs):
                    ...
                    if self.buffer.buffer_len() < self.demonstration_buffer_len:
                        self.buffer.push_demonstration_data((states, actions, reward, next_states, done), 1000)
                    else:
                ...
                if score.mean() >= best_score_mean:
                    ...
                    if best_score_mean >= 500 and torch.rand(1) < 0.2:
    ...
    def update_discrete(self, batch_size):
        state, action, reward, next_state, done = self.buffer.sample(batch_size)

        #--------------------------
        # DiscreteSAC Origin implementation
        #--------------------------
        alpha = self.alpha_log.exp().detach()
        with torch.no_grad():
            action2, log_prob2 = self.meta_actor_critic.evaluate(next_state)
